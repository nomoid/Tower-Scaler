...
public class GameOverMenu extends TSWindowMenu{
	...
	public GameOverMenu(GUIContext container, int x1, int y1, int x2,
			int y2, GameEngine ge){
	...
	public void init(GameContainer gc) throws SlickException{
		super.init(gc);
		TSTextLabel label = new TSTextLabel(gc, getWidth() / 2, getHeight() / 8, "Game Over");
		TSBoxLabel box = new TSBoxLabel(gc, label, Color.green, Color.black);
		tsftb = new TSFocusTextButton(gc, this, getWidth() / 2, getHeight() * 3 / 5, "Submit Score",
				Helper.getDefaultFont(), Color.cyan, Color.black, Color.black);
		addComponent(box);
		addComponent(tsftb);
	}
	...
	public void renderContainer(GameContainer gc, Graphics g, int x, int y) throws SlickException{
		...
		TSTextLabel name = new TSTextLabel(gc, getWidth() / 8, 
				getHeight() / 4, "Name:     " + ge.getName(), 
